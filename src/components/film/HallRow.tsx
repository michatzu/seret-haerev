...
export function HallRow({ label, screenings, film, venue }: { label: string; screenings: Screening[]; film: string; venue: string }) {
  ...
  return (
    <div className="flex items-center gap-2.5">
      ...
      <div className="flex flex-wrap gap-2">
        {shown.map((s) => (
          <TimePill key={s.id} s={s} big film={film} venue={venue} tag={s.dubbedLang === "ru" ? "רוסית" : s.dubbedLang === "en" ? "אנגלית" : s.attrs.includes("dubbed") ? "מדובב" : s.attrs.includes("3d") && label !== "3D" ? "3D" : undefined} />
        ))}
        {rest > 0 && (
